...
function slawsBuildSgp(){
  ...
  root.innerHTML=`<div class="signal warn"><b>Same-game correlation:</b> These percentages grade each leg separately. They must not be multiplied into an SGP hit probability. Check the live line, price, injuries, and game script before using them.</div><div class="sgp-picks">${chosen.map((x,i)=>`<div class="sgp-pick"><span class="sgp-rank">${i+1}</span><div><b>${x.p.name} over ${x.line.toFixed(1)} ${CFG[x.m].label.toLowerCase()}</b><div class="fine">${x.p.team} • ${Math.round(x.probability*100)}% individual estimate • ${x.s}/10 consistency</div></div></div>`).join("")}</div><button id="addSgp" class="action">Add all ${count} legs to Slip Builder</button>`;
